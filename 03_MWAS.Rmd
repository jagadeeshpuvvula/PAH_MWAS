...
```{r}
feature_table <- read_csv(paste0(path, "lc_ms_c18neg_mom.csv")) |>
  select(-c(3:10))|>
...
```{r}
feature_table <- read_csv(paste0(path, "lc_ms_hilicpos_mom.csv")) |>
  select(-c(3:10))|>
  mutate(across(3:last_col(), ~ log10(. + 1))) |>
  mutate(across(3:last_col(), ~ (. - mean(.)) / sd(.)))
```
...
```{r}
feature_table <- read_csv(paste0(path, "lc_ms_c18neg_baby.csv")) |>
  select(-c(3:10))|>
  mutate(across(3:last_col(), ~ log10(. + 1))) |>
  mutate(across(3:last_col(), ~ (. - mean(.)) / sd(.)))
```
...
```{r}
feature_table <- read_csv(paste0(path, "lc_ms_hilicpos_baby.csv")) |>
  select(-c(3:10))|>
  mutate(across(3:last_col(), ~ log10(. + 1))) |>
  mutate(across(3:last_col(), ~ (. - mean(.)) / sd(.)))
```
...
perform_PAH_MWAS <- function(feature_table, covariates, output_folder, mwas_file_name, cutoff_file_name) {
  ...
  for (variable in variables) {
    ...
    for (i in 1:nrow(feature_table)){
      ...
      current_results[i,] <- summary(lm(metabolite ~ get(variable) + mom_ppbmi + mom_age + pgtob + mom_edu_cat + race_cat ,
  ...
  write_csv(PAH_MWAS_result, file = output_file, row.names = FALSE)
  ...
  write_csv(fdr_cutoff_df, file = fdr_output_file, row.names = FALSE)
...
result<- perform_PAH_MWAS(feature_table = feature_table, 
                          ...
                          output_folder = "~/Documents/test",
                          mwas_file_name="mom_c18_neg.csv",
                          cutoff_file_name="mom_c18_neg_fdr_coff.csv")
```
