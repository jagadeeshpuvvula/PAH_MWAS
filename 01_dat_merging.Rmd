...
dt_survey <- read_sas("~/Documents/MWAS_home/Covariates/analysis_data.sas7bdat") |>
  ...
  select(participant_id, hh_inc_cat, stove, bbquse, dist_majhw_cat, regexer) |>
  mutate_all(factor) |>
...
dt_survey <- dt_survey |> left_join(ids, by="group") |>
...
dt_cov<- dt_cov |>
  ...
  select(-c(mom_edu, mom_race, regexer))

# join covariates and pah
dat<- left_join(mom_pah, dt_cov, by="group")|>
  filter(!is.na(participant_id_baby) | !is.na(participant_id_mom))

write_csv(dat, "~/Documents/MWAS_home/pj_dat/pah_fin_impu.csv")
```
